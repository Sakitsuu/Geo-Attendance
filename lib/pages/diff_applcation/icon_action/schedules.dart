...
class _SchedulesSiteState extends State<SchedulesSite> {
  ...
  Stream<String> _roleStream() {
    ...
    return _db.collection('users').doc(user.uid).snapshots().map((doc) {
      final role = (doc.data()?['role'] ?? 'worker').toString().toLowerCase();
      return role;
    });
  }

  Future<void> _openManagerEventDialog({
    required DateTime date,
    required String docId, // yyyy-MM-dd
        ...
        return StatefulBuilder(
          builder: (dialogContext, setLocal) {
            ...
            return AlertDialog(
              title: Text(
                existingTitle == null
                    ? "Add Event (${DateFormat('dd MMM yyyy').format(date)})"
                    : "Edit Event (${DateFormat('dd MMM yyyy').format(date)})",
              ),
              content: TextField(
              ...
              actions: [
                ...
                if (existingTitle != null)
                  TextButton(
                    onPressed: saving ? null : deleteEvent,
  ...
  //
  Future<void> _openViewDialog(DateTime date, String? title) async {
    final t = (title ?? '').trim();
    await showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(DateFormat('dd MMM yyyy').format(date)),
        content: Text(t.isEmpty ? 'No event.' : t),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<String>(
      ...
      builder: (context, roleSnap) {
        final role = (roleSnap.data ?? 'worker').toLowerCase();
                    ...
                    return GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return InkWell(
                          onTap: () async {
                            if (isManager) {
                              // manager can add/edit/delete
                              await _openManagerEventDialog(
                                date: date,
                                docId: key,
                                existingTitle: hasEvent ? events[key] : null,
                              );
                            } else {
                              // worker can view only
                              await _openViewDialog(
                                date,
                                hasEvent ? events[key] : null,
                              );
                            }
                          },
